...
describe('LabRequestsComponent', () => {
  ...
  beforeEach(async () => {
    const authServiceSpy = jasmine.createSpyObj('AuthService', ['getPatientId']);
  ...
  it('should load requests when patient ID is valid', () => {
    ...
    mockLabService.getByPatient.and.returnValue({
      subscribe: (callback: any) => callback(mockRequests)
    } as any);
  ...
  it('should handle load requests error', () => {
    mockAuthService.getPatientId.and.returnValue(1);
    mockLabService.getByPatient.and.returnValue({
      subscribe: (success: any, error: any) => error(new Error('Network error'))
    } as any);
