...
@Component({
  selector: 'app-patient-nutritionist-detail',
  templateUrl: './patient-nutritionist-detail.component.html',
  styleUrls: ['./patient-nutritionist-detail.component.scss']
})
export class PatientNutritionistDetailComponent implements OnInit {
  activeTab: 'about' | 'availability' | 'review' = 'about';
  NutritionistId: number = 0;
  Nutritionist: NutritionistProfile | null = null;
  ...
  goBack() {
    this.router.navigate(['/front/patient/nutritionists']);
  }

  loadNutritionistProfile() {
    this.NutritionistService.getNutritionist(this.NutritionistId).subscribe({
      next: (data: any) => {
        this.Nutritionist = {
          ...data,
      ...
      error: () => {
        this.error = 'Failed to load Nutritionist profile';
        this.isLoading = false;
      }
    });
  }
  ...
  private updatePredefinedReasons() {
    if (this.Nutritionist && this.Nutritionist.specialty) {
      const spec = this.Nutritionist.specialty;
  ...
  loadReviews() {
    this.reviewService.getReviews(this.NutritionistId).subscribe({
  ...
  loadMonthAvailabilities() {
    ...
    this.NutritionistService.getMonthAvailabilities(this.NutritionistId, this.formatDate(start), this.formatDate(end)).subscribe({
      next: (slots) => {
  ...
  loadSlotsForDate(date: Date) {
    ...
    this.NutritionistService.getAvailableSlots(this.NutritionistId, dateString).subscribe({
      next: (slots) => {
        const now = new Date();
  ...
  confirmBooking() {
    ...
    console.log('[DEBUG] PatientNutritionistDetailComponent: Submitting booking with payload:', JSON.stringify(payload));
    this.appointmentService.bookAppointment(payload).subscribe({
      next: (response) => {
        console.log('[DEBUG] PatientNutritionistDetailComponent: Booking success response:', response);
      ...
      error: (err) => {
        console.error('[ERROR] PatientNutritionistDetailComponent: Booking failed:', err);
  ...
  submitReview() {
    ...
    if (this.editingReviewId) {
      this.reviewService.updateReview(this.NutritionistId, this.editingReviewId, this.newReviewRating, this.newReviewComment, this.isAnonymous).subscribe({
        next: (updatedRev) => {
      ...
      this.reviewService.addReview(this.NutritionistId, this.newReviewRating, this.newReviewComment, this.isAnonymous).subscribe({
        next: (rev) => {
  ...
  deleteReviewConfirmed() {
    ...
    this.reviewService.deleteReview(this.NutritionistId, this.reviewToDeleteId).subscribe({
      next: () => {
        this.reviews = this.reviews.filter(r => r.id !== this.reviewToDeleteId);
        this.loadNutritionistProfile(); // Refresh average
