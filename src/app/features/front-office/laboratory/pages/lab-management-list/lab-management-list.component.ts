...
@Component({
  selector: 'app-lab-management-list',
  templateUrl: './lab-management-list.component.html',
  styleUrls: ['./lab-management-list.component.scss']
})
export class LabManagementListComponent implements OnInit, OnDestroy {
  ...
  getLabId(id: number): string { return 'LAB-' + String(id).padStart(3, '0'); }
  ...
  goToAdd(): void { this.router.navigate(['/front/laboratorystaff/laboratories/new']); }
  goToEdit(id: number): void { this.router.navigate(['/front/laboratorystaff/laboratories/edit', id]); }

  toggleActive(lab: LaboratoryResponse): void {
    this.labService.toggleActive(lab.id).subscribe({
      next: (updated) => {
        const update = (arr: LaboratoryResponse[]) => {
          const i = arr.findIndex(l => l.id === lab.id);
          if (i !== -1) arr[i] = updated;
        };
        update(this.labs); update(this.filteredLabs); update(this.pagedLabs);
  ...
  doDelete(): void {
    ...
    this.labService.delete(this.deleteTargetId).subscribe({
      ...
      error: () => { this.showConfirm = false; this.showSnackbar('Failed to delete.', 'error'); }
    });
  }

  showSnackbar(msg: string, type: 'success' | 'error'): void {
    this.snackbarMsg = msg; this.snackbarType = type; this.snackbarVisible = true;
